...
@section("content")
    <div class="m-5 p-5 bg-white">
        ...
        <br>
        <form method="post" action="{{route('supporter.updateInfo',['id' => $supporter->id])}}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
        ...
        <form method="post" action="{{route('supporter.updateAccount')}}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group row">

...
<a href="{{route('supporter.ticket.stete', ['state' => 'accept', 'id' => $messages[0]->ticket->id, 'color'=>'badge-success'])}}" class="btn btn-success waves-effect waves-light">Accept Ticket</a>
<a href="{{route('supporter.ticket.stete', ['state' => 'refuse', 'id' => $messages[0]->ticket->id, 'color'=>'badge-pink'])}}" class="btn btn-pink waves-effect waves-light">Refused Ticket</a>
@elseif ($messages[0]->ticket->state == 'open' || $messages[0]->ticket->state == 'accept')
<a href="{{route('supporter.ticket.stete', ['state' => 'close', 'id' => $messages[0]->ticket->id, 'color'=>'badge-purple'])}}" class="btn btn-purple waves-effect waves-light">Close Ticket</a>
@endif
...
<div class="card mt-4">
    <div class="card-body">
        ...
        <form method="post" action="{{route('supporter.message.store')}}" enctype="multipart/form-data">
            @csrf

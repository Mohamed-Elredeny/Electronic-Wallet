    ...
    <div class="wrapper d-flex justify-content-center flex-wrap">
        @foreach($categories as $cat)
          ...
          <div class="content">
              ...
              <div class="row">
                  <div class="col-sm-12 h5">
                      <center>
                          ...
                          <?php $count = count(\App\models\Product::where('category_id',$cat->id)->where('state','available')->get()) ?>
                          {{$count}}
                          {{__('home.card')}}
                          <hr>
                      </center>
            ...
            <div class="buttons">
                @if($count > 0)
              <a href="#" class="btn pt-3" data-toggle="modal" data-target="#course{{$cat->id}}" style="border: 0"  onclick="getModel('{{$cat->id}}','{{round($new_balance,2)}}','{{$count}}')">{{__('products.BuyNow')}}</a>
                @else
                <a href="#" class="btn pt-3" style="border: 0"  onclick="alert('هذا المنتج غير متاح الان')">{{__('products.BuyNow')}}</a>
              @endif
...
<!--@foreach($categories as $cat)-->
<script>
        function getModel(id,price,countt){
            document.getElementById('toty').innerHTML =`
    <div class="modal fade" id="course`+id+`" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="courseLabel`+id+`" aria-hidden="true">
        <form action="{{route('make.order')}}" method="post">
                @csrf
            <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
            <div class="modal-header backgroundColor text-white" style="border:none">
                <h5 class="modal-title" style="color: black" id="courseLabel`+id+`">Product NAme</h5>
                ...
                <div class="row">
                        ...
                        <div class="details mt-3">
                            <h6>{{__('products.ProductPrice')}}:
                                ...
                                @if(Session::get('currency') == 'USD')
                                    ...
                                    <input value="`+price+`" id="real_price" style="display:none" >
                                @else
                            ...
                            <h6 id="textChange">
                                ...
                                @if(Session::get('currency') == 'USD')
                                    <span id="plsChangeMe">`+price+` </span> {{Session::get('currency')}}
                    ...
                    <div class="col-lg-5 ml-3">
                        <div class="form-group row">
                            ...
                            <br>
                            <div class="col-sm-12">
                                <input class="form-control" type="number" max="`+parseInt(countt)+`" min="1" value="1" id="example_text_input_martina_md7ka" name="title" required onchange="chnageTotal()">
                                <input type="hidden" value="`+price+` " style="display:none">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{__('products.Close')}}</button>
                        ...
                        <input type="hidden" name="sad_category_id" value="`+id+`">
                        <input type="hidden" name="sad_price" value="`+price+` ">
